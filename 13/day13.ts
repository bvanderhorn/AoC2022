...
var sortedPackets: any[] = packets.map(el => parse(el)).sort((a,b) => isOrderCorrect([a,b]) === 'true' ? -1 : 1);
console.log(sortedPackets.slice(0,10).map(el => JSON.stringify(el)));
var dividerIndices: number[] = sortedPackets.map(el => JSON.stringify(el)).map((el, index) => [div1,div2].includes(el) ? index+1 : -1).filter(el => el >= 0);
